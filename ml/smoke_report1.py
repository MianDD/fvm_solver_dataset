...
def _pde_vec_13(law_index: int) -> np.ndarray:
    base = np.array([1.4, 1e-3, 1e-3, 2e-2, 2.5, 100.0, 1.0, 100.0, 8.0], dtype=np.float32)
    one_hot = np.zeros(3, dtype=np.float32)
    one_hot[int(law_index)] = 1.0
    return np.concatenate([base, one_hot, np.array([0.75 + 0.05 * law_index], dtype=np.float32)])


def _write_npz(path: Path, with_mask: bool, pde_dim: int = 13, law_index: int = 0) -> None:
...
def main() -> None:
    ...
    try:
        ...
        grid13.mkdir()
        grid9.mkdir()
        _write_npz(grid13 / "sim_0000.npz", with_mask=True, pde_dim=13, law_index=0)
        _write_npz(grid13 / "sim_0001.npz", with_mask=False, pde_dim=13, law_index=2)
        _write_npz(grid9 / "sim_0000.npz", with_mask=True, pde_dim=9, law_index=0)
        _write_npz(grid9 / "sim_0001.npz", with_mask=True, pde_dim=9, law_index=1)
        ...
        pde_normalizer = compute_pde_normalizer(pde_vectors_from_records(ds._cache))
        ...
        old_pred = torch.zeros_like(old_pde)
